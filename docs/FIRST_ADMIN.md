...
4. Set `ADMIN_EMAIL` to the same Auth email in local/Vercel server environment variables.
5. Sign in at `/login` using only the password and verify `/dashboard` loads.
